...
class Task(Cog_Extension):
    def __init__(self, *args, **kwargs):
        ...
        self.AdminRole = self.guild.get_role(db['Admin'])
    ...
    async def admin_check(self):
        if 21 <= now_time_info('hour') <= 23 or 0 <= now_time_info('hour') <= 6:
            ...
            for acc in Accs:
                user = await self.guild.fetch_member(acc[0])
                if acc[1] == 1:
                    await user.add_roles(self.AdminRole)
                elif acc[1] == 0:
                    await user.remove_roles(self.AdminRole)
